...
class Tester:
    def __init__(self, Gs, sigmas, image_pyramids, device):
        """
        * 테스트 관련 클래스
        :param Gs: 테스트 할 모델. 생성자
        :param sigmas: 시그마
    ...
    def running(self, nums_to_generate, output_dir, generated_folder_name):
        ...
        * 테스트 셋팅 및 진행
        ...
        mean = self.image_pyramids[-1][ConstVar.KEY_IMGPYR_MEAN]
        std = self.image_pyramids[-1][ConstVar.KEY_IMGPYR_STD]
    ...
    def _test(self):
        ...
        * 테스트 진행
        ...
        # 관련 변수 설정
        x = self.image_pyramids[highest_scale_num][ConstVar.KEY_IMGPYR_IMAGE]
        size = (self.nums_to_generate, *x.shape[1:])

        # 이미지 생성
        G = self.Gs[highest_scale_num].eval()
    ...
    def _get_prev(self, current_scale_num):
        ...
        * 현재 scale 의 이전 단계 결과물 가져오기
        ...
        # 이미지 생성 변수
        prev_G = self.Gs[prev_scale_num].eval()
