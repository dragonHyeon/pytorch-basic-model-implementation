...
class Trainer:
        ...
        * 학습 셋팅 및 진행
        ...
        for current_scale_num, (G, D) in enumerate(zip(self.Gs, self.Ds)):
            ...
            checkpoint_result_img_filepath = UtilLib.getNewPath(path=checkpoint_iter_dir,
                                                                add=ConstVar.CHECKPOINT_RESULT_IMG_FILE_NAME.format(current_scale_num + 1, len(self.image_pyramids)))
            self._draw_pic(fake_x=fake_x,
                           x=self.image_pyramids[current_scale_num][ConstVar.KEY_IMGPYR_IMAGE],
                           mean=self.image_pyramids[current_scale_num][ConstVar.KEY_IMGPYR_MEAN],
                           std=self.image_pyramids[current_scale_num][ConstVar.KEY_IMGPYR_STD],
    ...
    def _train(self, current_scale_num):
        ...
        * 학습 진행
        ...
        x = self.image_pyramids[current_scale_num][ConstVar.KEY_IMGPYR_IMAGE].to(self.device)
        ...
        prev = torch.zeros_like(input=x).to(self.device) if current_scale_num == 0 else self._get_prev(current_scale_num=current_scale_num, is_fixed=False, mode_train_test=ConstVar.MODE_TRAIN).to(self.device).detach()
        # 학습 시킬 현재 scale 의 이전 단계 결과물 (고정 노이즈 사용)
        fixed_prev = torch.zeros_like(input=x).to(self.device) if current_scale_num == 0 else self._get_prev(current_scale_num=current_scale_num, is_fixed=True, mode_train_test=ConstVar.MODE_TRAIN).to(self.device).detach()
        ...
        for j in range(3):
            ...
            # Total loss
            loss_G = loss_G_GAN + ConstVar.LAMBDA_REC * loss_G_rec
    ...
    def _eval(self, current_scale_num):
        ...
        * 테스트 진행
        ...
        prev = torch.zeros_like(input=x).to(self.device) if current_scale_num == 0 else self._get_prev(current_scale_num=current_scale_num, is_fixed=False, mode_train_test=ConstVar.MODE_TEST).to(self.device).detach()
        # 테스트 할 현재 scale 의 이전 단계 결과물 (고정 노이즈 사용)
        fixed_prev = torch.zeros_like(input=x).to(self.device) if current_scale_num == 0 else self._get_prev(current_scale_num=current_scale_num, is_fixed=True, mode_train_test=ConstVar.MODE_TEST).to(self.device).detach()
        ...
        # score 기록
        score = {
            ConstVar.KEY_SCORE_D: loss_D,
            ConstVar.KEY_SCORE_G: loss_G
        }
    ...
    def _get_prev(self, current_scale_num, is_fixed, mode_train_test):
        ...
        * 학습 시킬 현재 scale 의 이전 단계 결과물 가져오기
        ...
        upsample = nn.Upsample(size=self.image_pyramids[current_scale_num][ConstVar.KEY_IMGPYR_IMAGE].shape[2:])

        # 이전 단계 변수 설정
        prev_scale_num = current_scale_num - 1
        prev_G = self.Gs[prev_scale_num]
        if mode_train_test == ConstVar.MODE_TRAIN:
            prev_G.train()
        elif mode_train_test == ConstVar.MODE_TEST:
            prev_G.eval()
        prev_x = self.image_pyramids[prev_scale_num][ConstVar.KEY_IMGPYR_IMAGE]
        ...
        * 현재 저장하려는 모델이 가장 좋은 성능의 모델인지 여부 확인
        ...
        try:
            ...
            # 없다면 임의의 큰 숫자 (100000) 로 초기화
                ...
                self.best_score = ConstVar.INITIAL_BEST_SCORE
