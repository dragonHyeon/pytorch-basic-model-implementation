...
def save_checkpoint(filepath, Gs, Ds, sigmas, image_pyramids, is_best=False):
    ...
    * 체크포인트 저장
    ...
    # state 정보 담기
    state = {
        ConstVar.KEY_STATE_GS: Gs,
        ConstVar.KEY_STATE_DS: Ds,
        ConstVar.KEY_STATE_SIGMAS: sigmas,
        ConstVar.KEY_STATE_IMAGE_PYRAMIDS: image_pyramids
    }
...
def get_image_pyramids(filepath, max_value=250, min_value=25, scale_factor=4/3):
    """
    * 이미지 피라미드 만들기
    ...
    for w, h in zip(width_list, height_list):
        ...
        image_pyramids.append({
            ConstVar.KEY_IMGPYR_IMAGE: torch.unsqueeze(input=transformed_img, dim=0),
            ConstVar.KEY_IMGPYR_MEAN: mean,
            ConstVar.KEY_IMGPYR_STD: std
        })
